...
## Why
`.github/workflows/tests.yml` runs one command: `pytest tests/`. No Ruff, and
`pip install -e ".[dev]"` instead of the locked `uv` environment — so CI does not even test the
dependency set you develop against. The web app is a separate `SolarFab/nachtkarte` repository;
its own CI runs Vitest but not lint, typecheck or a production build.
...
- The separate web repository's job runs `npm ci`, lint, `tsc --noEmit`, unit tests and `next build`
...
## Impact
`.github/workflows/tests.yml` here, plus a separate PR in `SolarFab/nachtkarte` for its workflow,
`next.config.ts`, and whatever existing lint/type failures surface. No runtime behaviour change.
