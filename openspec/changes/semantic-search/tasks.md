...
- [x] 1.1 [db] SQL function or query for hybrid search: strict WHERE (active, upcoming, dates, category, facets, venue ilike, price) + optional `embedding <=> $qvec` ordering, LIMIT ≤ 20
- [x] 1.2 [web] Server-side query embedding via the OpenRouter gateway (same env config as pipeline); assert model id matches `events.embed_model`, else log + filter-only fallback
- [x] 1.3 [web] Verify anon-key/RLS posture: chat reads public events only

## 2. Tools

- [x] 2.1 [web] `search_events` tool: zod schema (category/subcategory enums generated from the taxonomy table), all params optional, compact result shape (no descriptions)
- [x] 2.2 [web] Geo params: lat+lng-together validation, radius default/clamp, haversine filter + distance ordering, `distance_km` in results
- [x] 2.3 [web] `get_event_details` tool: full row by id
- [x] 2.4 [web] Unit tests: arg validation (incl. incomplete geo), hard-constraint SQL (no paid event passes `free_entry`), facet-true-only semantics, unknown-price inclusion, ordering priority (query > distance > start_time)

## 3. Chat loop

- [x] 3.1 [web] Rewrite `chat/route.ts` on Vercel AI SDK `streamText` + `tools`, bounded steps; delete keyword/genre tables (completes `unify-taxonomy` §3.2)
- [x] 3.2 [web] System prompt: Berlin current time, grounding rules (tool results only, honest empty-result behaviour), untrusted-data rule for descriptions
- [x] 3.3 [web] Failure paths: embedding down → filter-only; tool error → brief error string to model
- [x] 3.4 [web] Keep streaming UX identical for the existing chat UI
...
- [x] 5.1 [web] Feature branch + preview deploy; side-by-side sanity check vs old chat
- [x] 5.2 [web] Merge after review; keep the old code path deletable in one revert
